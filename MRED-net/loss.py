...
def define_F(input_nc, netF, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, no_antialias=False,
             gpu_ids=[], initialize_weights=False):
    if netF == 'sample':
...
class PatchSampleF(nn.Module):
    ...
    def forward(self, patch_size, feats, num_patches, patch_ids=None, patch_local_ids=None, top_idxs=None,
                pixweght=None):
        # ipdb.set_trace()
        return_ids = []
        return_feats = []
            ...
            if patch_ids is not None:
                ...
                if pixweght is not None:
                    ...
                    if h < pixweght_map.shape[3]:
                        ...
                        pixweght_map = torch.nn.functional.avg_pool2d(pixweght_map, kernel_size=(scale, scale),
                                                                      stride=(scale, scale))
                ...
                local_id = get_local_index(feat_reshape, patch_id=patch_id, k_size=self.k_size[feat_id]).to(
                    feat_reshape.device)
