...
class SS2D(nn.Module):
    ...
    def forward_corev0(self, x: torch.Tensor):
        ...
        xs = torch.stack([x.view(B, -1, L), torch.flip(x.view(B, -1, L), dims=[-1]).contiguous()],
                             dim=1).view(B, 2, -1, L)
...
class token_SS2D(nn.Module):
    ...
    def forward_corev1(self, x: torch.Tensor):
        ...
        inv_y = torch.flip(out_y[:, 2:4], dims=[-1]).view(B, 2, -1, L)
        wh_y = torch.flip(out_y[:, 1], dims=[-1]).contiguous().view(B, -1, L)
        invwh_y = torch.flip(inv_y[:, 1], dims=[-1]).contiguous().view(B, -1, L)
...
class token_VSSBlock(nn.Module):
    def __init__(
            self,
            hidden_dim: int = 0,
            d_output: int = 64,
        ...
        super().__init__()
        self.ln_1 = norm_layer(hidden_dim)
        self.self_attention = token_SS2D(d_model=hidden_dim, d_output=d_output,dropout=attn_drop_rate, d_state=d_state, **kwargs)
        self.drop_path = DropPath(drop_path)
...
class T2T_module(nn.Module):
    ...
    def __init__(self, img_size=64, tokens_type='performer', in_chans=1, embed_dim=256, token_dim=64):
        ...
        if tokens_type == 'transformer':
            ...
            self.attention1 = token_VSSLayer(dim=in_chans*7*7, output_dim=token_dim, depth=1, downsample=None)
            self.attention2 = token_VSSLayer(dim=token_dim*3*3, output_dim=token_dim, depth=1, downsample=None)
    ...
    def forward(self, x):
        ...
        x = self.soft_split1(x)  # [64,64,29,29]->[64,576,625]   576=3*3*64

        # CTformer module B
        x = self.attention2(x.transpose(1, 2))  # [64,576,625]->[64,625,64]
        res_22 = x
        B, new_HW, C = x.shape
        ...
        x = self.project(x.transpose(1, 2))  ## [64,576,529]->[64,529,64]
...
class Token_back_Image(nn.Module):
    ...
    def __init__(self, img_size=64, tokens_type='transformer', in_chans=1, embed_dim=256, token_dim=64, kernel=32,
        ...
        if tokens_type == 'transformer':
            ...
            self.attention1 = token_VSSLayer(dim=token_dim, output_dim=in_chans*7*7, depth=1, downsample=None)
            self.attention2 = token_VSSLayer(dim=token_dim, output_dim=token_dim*3*3, depth=1, downsample=None)
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[1], depths_decoder=[1],
                 dims=[256], dims_decoder=[256], d_state=16, drop_rate=0.,
        ...
        if self.ape:
            ...
            self.absolute_pos_embed = nn.Parameter(torch.zeros(1, 529, self.embed_dim))
